...
function getMongoConfig() {
  ...
  const dbName = process.env.MONGODB_DB_NAME || envValues.MONGODB_DB_NAME || DEFAULT_DB_NAME;
  return { uri, dbName };
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--limit' || a === '-l') args.limit = Number(argv[++i] ?? '0');
...
const COLLECTION_PROBES = {
  ...
  'entity-mentions': [
    ...
    'factsExtracted.hash',
    // `factsExtracted.value` can be any JSON-ish structure; include it anyway.
...
async function findBadDocsInCollection(coll, collName, probeFields, limit, maxErrors) {
  ...
  while (await idCursor.hasNext()) {
    ...
    try {
      ...
      for (const field of probeFields) {
        try {
          await coll.findOne({ _id }, { projection: fieldsToProjection([field]) });
        } catch (fieldErr) {
          badFields.push(field);
          console.log(`  field: ${field} -> ${fieldErr && fieldErr.message ? fieldErr.message : String(fieldErr)}`);
        }
      }

      if (badFields.length === 0) {
        console.log('  note: error reproduced, but none of the probe fields isolated it (try adding more fields).');
      }
...
`);
  ...
  let collectionsToScan = ['entities', 'entity-mentions', 'entity-descriptions'];
  ...
  try {
    ...
    for (const collName of collectionsToScan) {
      ...
      await findBadDocsInCollection(coll, collName, probeFields, args.limit, args.maxErrors);
    }
  } finally {
