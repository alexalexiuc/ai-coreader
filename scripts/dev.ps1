...
& { $ErrorActionPreference = "Continue"; docker compose -f $composeFile --profile stack down 2>$null | Out-Null }
...
Start-Process $shell -ArgumentList @(
    "-NoExit",
    "-Command",
    "docker compose -f '$composeFile' --profile infra up --remove-orphans"
...
function Start-DevProcess {
    ...
    Start-Process $shell -ArgumentList @("-NoExit", "-Command", $Command) -WorkingDirectory $WorkingDir
}
...
Write-Host "  (Close each terminal window individually to stop Next.js and Go worker)" -ForegroundColor DarkGray
