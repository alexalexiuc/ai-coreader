...
function startDockerContainer() {
  ...
  execFileSync(
    "docker",
    ["run", "-d", "--name", CONTAINER_NAME, "-p", `${DEFAULT_PORT}:${DEFAULT_PORT}`, "mongo:7"],
    { stdio: "inherit" }
  );
}
...
async function main() {
  ...
  if (!["localhost", "127.0.0.1"].includes(host) || port !== DEFAULT_PORT) {
    console.error(
      `MongoDB not reachable at ${host}:${port}. Start it manually or set MONGODB_URI to a reachable host.`
    );
    process.exit(1);
  }
  ...
  if (!hasDocker()) {
    console.error(
      "MongoDB is not running and Docker is unavailable. Start MongoDB locally or install Docker."
    );
    process.exit(1);
  }
